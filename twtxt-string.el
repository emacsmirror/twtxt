...
(defun twtxt--string-format-mentions-to-markdown-links (text)
...
Example: @<my alias http://example.com> → [my alias](http://example.com)."
  (let ((regex "@<\\([^>]+\\)>"))
	...
	(let* ((source (match-string 0 text))
	       ...
	       (url (car (last items))))
	  (twtxt--string-format-mentions-to-markdown-links (replace-regexp-in-string (regexp-quote source)
...
Returns the converted text as a string."
  (if twtxt--pandoc-p
      (with-temp-buffer
	(insert (twtxt--string-format-mentions-to-markdown-links md-text))
